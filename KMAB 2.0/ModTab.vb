...
Public Class ModTab
    ...
    Private Sub ModsTab_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Timer1.Start()
        If Not System.IO.Directory.Exists("C:\KimuSoft\KuBukkit\server\" + Replace(Servername.Text, " ", "_") + "\mods") Then
            MkDir("C:\KimuSoft\KuBukkit\server\" + Replace(Servername.Text, " ", "_") + "\mods")
        End If
        DirSer()
        FlatButton4.Text = GetIniValue("slot1", "modsvalue", "C:\KimuSoft\KuBukkit\Serverinfor.ini") + " Mods"
        Dim Laugh As String = System.IO.Directory.GetFiles("C:\KimuSoft\KuBukkit\server\" + Replace(Servername.Text, " ", "_") + "\mods").Length
        SetIniValue("slot" + SlotNumber.Text, "modsvalue", Laugh, "C:\KimuSoft\KuBukkit\Serverinfor.ini")
        FlatButton4.Text = Laugh + " Mods"
    ...
    Private Sub DirSer()
        ListView1.Clear()
        Dim Dl As String
        Dim di As New IO.DirectoryInfo("C:\KimuSoft\KuBukkit\server\" + Replace(Servername.Text, " ", "_") + "\mods")
    ...
    '    If Not A = "" Then
    '        Kill("C:\KimuSoft\KuBukkit\server\" + Replace(Servername.Text, " ", "_") + "\mods\" + A)
    '        DirSer()
    '    ElseIf A = "" Then
    '        MsgBox("삭제할 플러그인을 선택 해주세요.", vbInformation)
    '    ElseIf Not System.IO.File.Exists("C:\KimuSoft\KuBukkit\server\" + Replace(Servername.Text, " ", "_") + "\mods\" + A) Then
    ...
    Private Sub FlatButton6_Click(sender As Object, e As EventArgs) Handles FlatButton6.Click
        MsgBox("슬프게도, 이 프로그램은 아직 플러그인을 삭제할 줄을 모릅니다.") 'ListView1.SelectedItems.Count)
        For Each Item As ListViewItem In ListView1.SelectedItems
            File.Delete("C:\KimuSoft\KuBukkit\server\" + Replace(Servername.Text, " ", "_") + "\mods\" + Item.Text.ToString())
            Item.Remove()
        Next
        DirSer()
    End Sub
    ...
    Private Sub FlatButton3_Click(sender As Object, e As EventArgs) Handles FlatButton3.Click
        Shell("explorer C:\Kimusoft\KuBukkit\server\" + Replace(Servername.Text, " ", "_") + "\mods", vbNormalFocus)
    End Sub

    Private Sub FlatButton1_Click(sender As Object, e As EventArgs) Handles FlatButton1.Click
        If (OpenFileDialog1.ShowDialog() = DialogResult.OK) Then
            If System.IO.File.Exists("C:\KimuSoft\KuBukkit\server\" + Replace(Servername.Text, " ", "_") + "\mods\" + OpenFileDialog1.SafeFileName) Then
                MsgBox("아무래도, 이미 적용된 모드인 것 같습니다.", vbInformation)
                Exit Sub
            End If
            My.Computer.FileSystem.CopyFile(OpenFileDialog1.FileName, "C:\KimuSoft\KuBukkit\server\" + Replace(Servername.Text, " ", "_") + "\mods\" + OpenFileDialog1.SafeFileName)
            DirSer()
        End If
        'If System.IO.File.Exists(OpenFileDialog1.FileName) Then
        '    Dim ini As Long = GetIniValue("slot" + SlotNumber.Text, "modsvalue", "C:\KimuSoft\KuBukkit\Serverinfor.ini")
        '    SetIniValue("slot" + SlotNumber.Text, "modsvalue", ini + 1, "C:\KimuSoft\KuBukkit\Serverinfor.ini")
        '    FlatButton4.Text = GetIniValue("slot" + SlotNumber.Text, "modsvalue", "C:\KimuSoft\KuBukkit\Serverinfor.ini") + " Mods"
        'End If
    End Sub
    ...
    Private Sub ListView1_DragDrop(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles ListView1.DragDrop
        ...
        For Each Filename As String In drop
            ...
            If System.IO.File.Exists("C:\KimuSoft\KuBukkit\server\" + Replace(Servername.Text, " ", "_") + "\mods\" + Path.GetFileName(Filename)) Then
                MsgBox("아무래도 이미 적용된 모드인 것 같습니다.", vbInformation)
                Exit Sub
            End If
            If Not fileExtension = ".jar" Or Not fileExtension = ".zip" Then
                ...
                End If
            End If
            My.Computer.FileSystem.CopyFile(Filename, "C:\KimuSoft\KuBukkit\server\" + Replace(Servername.Text, " ", "_") + "\mods\" + Path.GetFileName(Filename))
            'Dim fileExtension As String = Path.GetExtension(Filename)
